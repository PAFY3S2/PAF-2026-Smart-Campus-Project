...
const Sidebar = ({ isOpen, setOpen }) => {
  ...
  const coreItems = [
    { name: 'Dashboard', path: user?.role === 'TECHNICIAN' ? '/technician/dashboard' : '/dashboard', icon: LayoutDashboard },
    { name: 'Infrastructure', path: '/resources', icon: Box },
    { name: 'Profile', path: user?.role === 'TECHNICIAN' ? '/technician/profile' : '/profile', icon: UserIcon },
  ];
  ...
  const adminItems = [
    ...
    { name: 'Resource Ops', path: '/resources', icon: shieldIcon(Monitor) },
    { name: 'User Directory', path: '/admin/users', icon: shieldIcon(Users) },
  ];

  const technicianItems = [
    { name: 'Hub Overview', path: '/technician/dashboard', icon: LayoutDashboard },
    { name: 'In Progress', path: '/technician/in-progress', icon: Clock },
    { name: 'Resolved', path: '/technician/resolved', icon: CheckCircle },
    { name: 'Priority Queue', path: '/technician/priority', icon: Flag },
    { name: 'Closed History', path: '/technician/closed', icon: Layers },
  ];
      ...
      <div className="p-8 flex-1 overflow-y-auto custom-scrollbar">
        ...
        <nav className="space-y-10">
          ...
          {user?.role === 'TECHNICIAN' && (
            <div>
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-700 uppercase tracking-[0.4em] mb-4 ml-4">Technical Operations</p>
              <div className="space-y-1">
                {technicianItems.map(item => <NavItem key={item.path} {...item} />)}
              </div>
            </div>
          )}
        </nav>
        ...
        <div className="flex items-center space-x-4 mb-6">
           <div className="relative">
             <img 
               ...
               alt="" 
               className="w-10 h-10 rounded-xl border border-slate-200 dark:border-slate-800 shadow-md" 
             />
