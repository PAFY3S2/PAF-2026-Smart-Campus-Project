  ...
  return (
    <div className="min-h-screen bg-[#F1F3F5] dark:bg-slate-950 p-8 pt-6 font-sans">
      <div className="max-w-[1280px] mx-auto">
        ...
        <button 
          onClick={() => navigate(-1)}
          className="flex items-center space-x-2 text-slate-500 hover:text-[#142B5D] dark:hover:text-[#F5AB24] transition-colors mb-6 group"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between mb-8 gap-6">
          <div className="space-y-1 text-slate-900 dark:text-white">
             ...
             <div className="flex items-center space-x-2 text-[15px] font-medium text-slate-500">
                ...
                <span>Submitted {formatDistanceToNow(new Date(ticket.createdAt), { addSuffix: true })} by {ticket.userDetails?.name || 'Anonymous'}</span>
             </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="space-y-8">
             <RequesterDetails user={ticket.userDetails} />
