  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl p-8 border border-slate-100 dark:border-slate-800 shadow-sm">
      ...
      <div className="flex items-center space-x-4 mb-8">
        ...
        <div>
           <h3 className="text-[17px] font-bold text-[#1F2937] dark:text-white leading-tight">{user.name || 'Marcus Johnson'}</h3>
           <p className="text-[12px] font-medium text-slate-500 mt-0.5">{user.department || 'Chemistry Dept.'} {user.role || 'Senior'}</p>
        </div>
      </div>

      <div className="space-y-5">
        <DetailItem icon={Mail} value={user.email} />
        <DetailItem icon={Phone} value={user.phone || '+1 (555) 234-9081'} />
        <DetailItem icon={MapPin} value={user.location || 'Science Building, Lab 304'} />
      </div>
    </div>
  );
};
